...
for isi = 1:length(R_b)
    semilogy(C_n_2, ber_ana(isi, :), string(ana(isi)));
    hold on 
    semilogy(C_n_2, ber_sim(isi, :), string(sim(isi))); 
end 
% legend('$R_b = 100 $ Mb/s analysis','$R_b = 100 $ Mb/s simulation',...
%     '$R_b = 200 $ Mb/s analysis','$R_b = 200 $ Mb/s simulation', ...
%     '$R_b = 500 $ Mb/s analysis','$R_b = 500 $ Mb/s simulation', ...
%     '$R_b = 1 $ Gb/s analysis','$R_b = 1 $ Gb/s simulation', ...
%     '$R_b = 2$ Gb/s analysis','$R_b = 2 $ Gb/s simulation')
grid on 
ylim([1e-6 1])
